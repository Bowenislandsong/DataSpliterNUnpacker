...
public class FileAssert {
    ...
    private static Integer MAX; // max number of files in a folder
    private static String MaxFilePath;
        ...
        if (BringAllToRoot){
            ...
            for (File file : folder.listFiles()) {
                ...
                if (file.isDirectory())
                    ...
                    if (file.getParent().equals(MaxFilePath)){
                        MoveFile(file, dirName(file, fNn));
                    }else{
                        for (int i = 0; i< Nnode;i++){
                            CPFile(file, dirName(file, i));
                        }
                    }
                }
            }
    ...
    private static File dirName(File file, Integer fin) {
        return new File(Integer.toString(fin) + "/" + file.getPath().replaceFirst(ResorRootDir.toString(),""));
    }
    ...
    public static void zipinside(File file) throws ZipException {
        if (file.isDirectory() && file.list().length > 0) {
            ...
            //zipFile.createZipFileFromFolder(file,parameters,false,0);
            for (File f:file.listFiles()) {
                if(f.isDirectory()){
    ...
    private static void CPFile(File src, File tar) {
        if(!src.exists())
            return;
        try{
            if (!BringAllToRoot) {
